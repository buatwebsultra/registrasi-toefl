...
<style nonce="{{ $csp_nonce ?? '' }}">
    ...
    .bg-metric-blue { background: #eff6ff !important; color: #2563eb !important; }
    .bg-metric-amber { background: #fffbeb !important; color: #d97706 !important; }
    .bg-metric-emerald { background: #ecfdf5 !important; color: #059669 !important; }
    .bg-metric-indigo { background: #eef2ff !important; color: #4f46e5 !important; }
    .bg-metric-rose { background: #fff1f2 !important; color: #be123c !important; }
    .bg-metric-rose-dark { background: #fff1f2 !important; color: #e11d48 !important; }
    
    .min-w-60 { min-width: 60px !important; }
    .h-4px { height: 4px !important; }
    .w-150px { width: 150px !important; }
    .opacity-50 { opacity: 0.5 !important; }
    .progress-bar-indigo { background: #6366f1 !important; }
</style>
    ...
    <div class="dashboard-header">
        <div class="row align-items-center">
            ...
            <div class="col-lg">
                ...
                <p class="lead opacity-75 mb-4">Kelola jadwal dan pendaftaran tes TOEFL dari dashboard terintegrasi Anda.</p>
                <div class="d-flex flex-wrap gap-2">
                    <span class="admin-role-badge">
                        <i class="fas fa-user-shield me-2"></i> {{ Auth::user()->role === 'superadmin' ? 'Super Admin' : (Auth::user()->role === 'admin' ? 'Admin' : 'Operator') }}
                    </span>
                    <span class="admin-role-badge">
            ...
            <div class="col-lg-4 text-center text-lg-end mt-4 mt-lg-0">
                <div class="btn-group shadow-sm">
                    @if(Auth::user()->isSuperAdmin())
                        <a href="{{ route('admin.users.index') }}" class="btn btn-light px-4 py-2 border-end border-light-subtle">
                            <i class="fas fa-users-cog me-2"></i> Manajemen
                        </a>
                        <a href="{{ route('admin.logs.index') }}" class="btn btn-light px-4 py-2 border-end border-light-subtle">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="metric-card p-4">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <a href="{{ route('admin.participants.pending') }}" class="btn btn-sm btn-outline-warning rounded-pill px-3">Detail</a>
                </div>
            </div>
        </div>
        <div class="col-xl-3 col-md-6">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.schedule.create') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control-premium w-100" name="date" min="{{ date('Y-m-d') }}" required>
                        </div>
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                <label class="form-label fw-bold small text-muted text-uppercase">Waktu</label>
                                <input type="time" class="form-control-premium w-100" name="time" value="08:00" required>
                            </div>
                            <div class="col-6">
                                <label class="form-label fw-bold small text-muted text-uppercase">Kapasitas</label>
                                <input type="number" class="form-control-premium w-100" name="capacity" min="1" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control-premium w-100" name="room" required>
                                ...
                                @for($i = 1; $i <= 10; $i++)
                                    <option value="UPT-{{ str_pad($i, 2, '0', STR_PAD_LEFT) }}">UPT-{{ str_pad($i, 2, '0', STR_PAD_LEFT) }}</option>
                                @endfor
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small text-muted text-uppercase">Kategori</label>
                            <input type="text" class="form-control-premium w-100" name="category" value="TOEFL-LIKE" required>
                        </div>
            ...
            <div class="premium-card">
                ...
                <div class="card-body p-4">
                    <div class="d-grid gap-3">
                        <a href="{{ route('admin.faculties.index') }}" class="btn btn-outline-light text-dark border p-3 d-flex align-items-center justify-content-between rounded-3 transition-all hover:bg-light">
                            <span class="fw-bold"><i class="fas fa-university me-3 text-primary"></i>Kelola Fakultas</span>
                            <i class="fas fa-chevron-right small opacity-50"></i>
                        </a>
                        <a href="{{ route('admin.study-programs.index') }}" class="btn btn-outline-light text-dark border p-3 d-flex align-items-center justify-content-between rounded-3 transition-all hover:bg-light">
                            <span class="fw-bold"><i class="fas fa-book me-3 text-info"></i>Kelola Program Studi</span>
                            <i class="fas fa-chevron-right small opacity-50"></i>
                        </a>
                        <a href="{{ route('admin.gallery.index') }}" class="btn btn-outline-light text-dark border p-3 d-flex align-items-center justify-content-between rounded-3 transition-all hover:bg-light">
                            <span class="fw-bold"><i class="fas fa-images me-3 text-warning"></i>Kelola Galeri (Slider)</span>
                            <i class="fas fa-chevron-right small opacity-50"></i>
                        </a>
                    </div>
                </div>
                ...
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        <form action="{{ route('admin.dashboard') }}" method="GET" class="d-flex gap-2">
                            <input type="date" name="search_date" class="form-control form-control-sm border-radius-lg" value="{{ $searchDate }}">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <div class="rounded-3 p-2 me-3 text-center min-w-60 bg-metric-indigo">
                                        ...
                                        <div>
                                            <div class="fw-bold">{{ $schedule->time ? \Carbon\Carbon::parse($schedule->time)->format('H:i') : '08:00' }}</div>
                                            <div class="small text-muted"><i class="fas fa-map-marker-alt me-1"></i>{{ $schedule->room }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="fw-bold">{{ $schedule->participants_count }} / {{ $schedule->capacity }}</div>
                                    <div class="progress mt-1 h-4px">
                                        @php
                                            $percent = $schedule->capacity > 0 ? ($schedule->participants_count / $schedule->capacity) * 100 : 0;
                                        @endphp
                                        <div class="progress-bar progress-bar-indigo dashboard-progress" data-percent="{{ $percent }}"></div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    @if(Auth::user()->isAdmin())
                                        <button class="action-btn bg-metric-amber border-0" data-bs-toggle="modal" data-bs-target="#editModal{{ $schedule->id }}" title="Edit Jadwal">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="action-btn bg-metric-rose-dark border-0" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $schedule->id }}" title="Hapus Jadwal">
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <img src="https://cdni.iconscout.com/illustration/premium/thumb/no-data-found-8867280-7223938.png" alt="No data" class="w-150px opacity-50">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-body p-4 text-center">
                ...
                <p class="text-muted mb-4">Anda akan menghapus jadwal di ruangan <strong>{{ $schedule->room }}</strong>. Tindakan ini tidak dapat dibatalkan.</p>
                <div class="bg-light p-3 rounded-3 mb-4 text-start">
                    ...
                    <input type="text" class="form-control-premium w-100" id="confirmRoomName{{ $schedule->id }}" placeholder="Ketik '{{ $schedule->room }}' untuk menghapus">
                    <div id="confirmError{{ $schedule->id }}" class="text-danger small mt-1 d-none">Nama ruangan tidak sesuai</div>
                </div>
                <div class="d-flex gap-3 mt-4">
                    <button type="button" class="btn btn-light flex-grow-1 py-2 rounded-3" data-bs-dismiss="modal">Batal</button>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <form action="{{ route('admin.schedule.update', $schedule->id) }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-muted text-uppercase">Tanggal Tes</label>
                        <input type="date" class="form-control-premium w-100" name="date" value="{{ $schedule->date->format('Y-m-d') }}" required>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-bold small text-muted text-uppercase">Waktu</label>
                            <input type="time" class="form-control-premium w-100" name="time" value="{{ $schedule->time ? \Carbon\Carbon::parse($schedule->time)->format('H:i') : '08:00' }}" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-bold small text-muted text-uppercase">Kapasitas</label>
                            <input type="number" class="form-control-premium w-100" name="capacity" value="{{ $schedule->capacity }}" min="1" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-muted text-uppercase">Ruangan</label>
                        <select class="form-control-premium w-100" name="room" required>
                            @for($i = 1; $i <= 10; $i++)
                                ...
                                <option value="{{ $roomValue }}" {{ $schedule->room == $roomValue ? 'selected' : '' }}>{{ $roomValue }}</option>
                            @endfor
                        </select>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control-premium w-100" name="signature_name" value="{{ $schedule->signature_name }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-bold small text-muted text-uppercase">NIP Penandatangan</label>
                            <input type="text" class="form-control-premium w-100" name="signature_nip" value="{{ $schedule->signature_nip }}">
                        </div>
                    </div>
                </div>
